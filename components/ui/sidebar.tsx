...
// Tailwind v4 requires var() wrapper for CSS variables in arbitrary values
...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
  ...
  // Hover state for desktop icon-collapsible sidebar (expands on hover when collapsed)
  ...
  // Compute visual state: expanded if open OR hovering (when collapsed)
  const visualState = React.useMemo(() => {
    if (open) return "expanded";
    if (!open && isHovering && collapsible === "icon" && !isMobile) return "expanded";
    return "collapsed";
  }, [open, isHovering, collapsible, isMobile]);

  // Handle mouse enter: expand on hover when collapsed (desktop icon-collapsible only)
  ...
  // Handle mouse leave: delayed collapse (1s delay)
  ...
  // Reset hover state when open state changes
  React.useEffect(() => {
    if (open) {
      setIsHovering(false);
      if (collapseTimeoutRef.current) {
    ...
    >
      ...
      <div
        className={cn(
          ...
          // Faster transition when hovering (150ms)
          isHovering && !open && collapsible === "icon" && "duration-150",
...
const SidebarPinTrigger = React.forwardRef<HTMLButtonElement, React.ComponentProps<"button">>(
  ({ className, ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={cn(
          "absolute -right-3 w-6 h-6 rounded-full bg-sidebar border-2 border-sidebar-border",
          "flex items-center justify-center hover:bg-sidebar-accent z-10 shadow-md",
          "transition-all duration-200 ease-linear",
          // Position lower when collapsed to avoid overlapping logo icon
          state === "collapsed" ? "top-20" : "top-8",
          className,
        )}
      ...
      >
        <ChevronLeft
          className={cn(
            "w-3 h-3 text-sidebar-foreground transition-transform duration-200 ease-linear",
            open && "rotate-180",
          )}
        />
      </button>
...
SidebarPinTrigger.displayName = "SidebarPinTrigger";
