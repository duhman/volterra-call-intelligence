...
Deno.serve(async (req) => {
  ...
  const supabase = createClient(supabaseUrl, supabaseServiceKey, { db: { schema: "call_intelligence" } });
  ...
  try {
    ...
    if (!providedPassword) {
      ...
      return new Response(JSON.stringify({ error: 'Unauthorized - no password provided' }), {
        status: 401,
    ...
    if (!passwordsMatch) {
      console.log(`[admin-api] Password mismatch - provided length: ${providedPassword.length}, expected length: ${adminPassword.length}`);
    ...
    if (method === 'GET' && (path === '/calls' || path === '')) {
      ...
      if (search) {
        query = query.or(`from_number.ilike.%${search}%,to_number.ilike.%${search}%,agent_email.ilike.%${search}%`);
      }
    ...
    if (method === 'POST' && path === '/calls/bulk-reprocess') {
      ...
      for (const call of skippedCalls || []) {
        ...
        }).catch(err => console.error('Failed to trigger reprocess for call', call.id, err));
      }
    ...
    if (method === 'POST' && path === '/calls/bulk-regenerate-summaries') {
      ...
      if (!completedCalls || completedCalls.length === 0) {
        return new Response(JSON.stringify({ status: 'complete', count: 0, message: 'No completed calls found' }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
      ...
      const { data: callsWithTranscriptions } = await supabase
        .from('transcriptions')
        .select('call_id')
        .in('call_id', completedCalls.map(c => c.id));

      const callIds = callsWithTranscriptions?.map(t => t.call_id) || [];
      const count = callIds.length;

      if (count === 0) {
        return new Response(JSON.stringify({ status: 'complete', count: 0, message: 'No completed calls with transcriptions found' }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }

      console.log(`Bulk regenerating summaries for ${count} completed calls with transcriptions`);
      ...
      for (const callId of callIds) {
        try {
          ...
          const summaryResp = await fetch(`${supabaseUrl}/functions/v1/generate-summary`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
          ...
          const hubspotResp = await fetch(`${supabaseUrl}/functions/v1/hubspot-sync`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${supabaseServiceKey}`,
              'Content-Type': 'application/json',
          ...
          console.log(`HubSpot sync response for ${callId}: ${hubspotResp.status}`);
        } catch (err) {
    ...
    if (method === 'POST' && path.match(/\/calls\/[^/]+\/reprocess/)) {
      ...
      return new Response(JSON.stringify({ status: 'reprocessing', callId, manualRecordingId }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (method === 'GET' && path === '/stats') {
      const [
        ...
        supabase.from('calls').select('*', { count: 'exact', head: true }).eq('status', 'pending'),
        supabase.from('calls').select('*', { count: 'exact', head: true }).eq('status', 'failed'),
        supabase.from('calls').select('*', { count: 'exact', head: true })
    ...
    // GET /webhook-url - Get the webhook URL for Telavox setup
    ...
    // GET /debug/telavox - Debug Telavox API connection and response
    if (method === 'GET' && path === '/debug/telavox') {
      const telavoxApiKey = Deno.env.get('TELAVOX_API_KEY');
      
      if (!telavoxApiKey) {
        return new Response(JSON.stringify({ error: 'TELAVOX_API_KEY not configured' }), {
          status: 500,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }

      console.log('Debug: Testing Telavox API connection...');
      ...
      callHistoryUrl.searchParams.set('fromDate', yesterday.toISOString().split('T')[0]);
      callHistoryUrl.searchParams.set('toDate', now.toISOString().split('T')[0]);
      ...
      try {
        ...
        const withRecordings = allCalls.filter((c: any) => c.recordingId && c.recordingId !== '0');
        const withoutRecordings = allCalls.filter((c: any) => !c.recordingId || c.recordingId === '0');

        // Get field names from first call of each type
        const sampleFields = allCalls.length > 0 ? Object.keys(allCalls[0]) : [];
        ...
        return new Response(JSON.stringify({
          ...
          sample_calls_with_recording: withRecordings.slice(0, 3).map((c: any) => ({
          ...
          sample_calls_without_recording: withoutRecordings.slice(0, 3).map((c: any) => ({
            number: c.number || c.numberE164,
            dateTime: c.dateTimeISO || c.datetimeISO || c.datetime,
            recordingId: c.recordingId,
            callId: c.callId,
        ...
        console.error('Debug: Telavox API fetch error:', fetchError);
        return new Response(JSON.stringify({
          error: 'Failed to fetch from Telavox API',
    ...
    // GET /debug/telavox-key/:email - Test a specific agent's API key
    // This helps diagnose why recordings aren't being found for an agent
    if (method === 'GET' && path.startsWith('/debug/telavox-key/')) {
      const email = decodeURIComponent(path.replace('/debug/telavox-key/', '')).toLowerCase().trim();
      
      console.log(`Debug: Testing Telavox API key for agent: ${email}`);
      ...
      if (!agentKey) {
        return new Response(JSON.stringify({ 
          ...
          suggestion: 'Add an API key for this agent in Settings > Agent API Keys'
        }), {
          status: 404,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      ...
      try {
        ...
        if (!response.ok) {
          ...
          return new Response(JSON.stringify({
            ...
            suggestion: 'Create a new API token for this user in Telavox: My Account > Username and Password > Manage tokens'
          }), {
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
        ...
        return new Response(JSON.stringify({
          agent_email: agentKey.agent_email,
          error: 'Failed to connect to Telavox API',
    ...
    if (method === 'POST' && path === '/blocked-numbers') {
      ...
      if (error) {
        if (error.code === '23505') { // Unique violation
          return new Response(JSON.stringify({ error: 'Number already blocked' }), {
            status: 409,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
    ...
    if (method === 'GET' && path === '/telavox-keys') {
      ...
      const maskedKeys = (data || []).map((k: any) => ({
        ...k,
        api_key: k.api_key ? `${k.api_key.slice(0, 4)}...${k.api_key.slice(-4)}` : '',
      }));
    ...
    if (method === 'POST' && path === '/telavox-keys') {
      ...
      if (!agentEmail || !apiKey) {
        return new Response(JSON.stringify({ error: 'agentEmail and apiKey are required' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
      ...
      if (error) {
        if (error.code === '23505') {
          return new Response(JSON.stringify({ error: 'Agent already has an API key configured' }), {
            status: 409,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
    ...
    if (method === 'PUT' && path.match(/^\/telavox-keys\/[^/]+$/)) {
      ...
      if (displayName !== undefined) updateData.display_name = displayName?.trim() || null;
      if (hubspotUserId !== undefined) updateData.hubspot_user_id = hubspotUserId?.trim() || null;
