-- Migration: 014_slack_consent_system.sql
-- Purpose: Add Slack-based transcription consent workflow
-- Date: 2026-01-09
SET
...
-- ==========================================
-- Maps Telavox agent_user_id to Slack user_id for DM routing
...
CREATE POLICY "Service role only on agent_slack_mappings" ON call_intelligence.agent_slack_mappings FOR ALL TO authenticated,
anon USING (auth.role () = 'service_role')
WITH
...
-- ==========================================
-- Tracks pending/resolved consent requests
...
CREATE POLICY "Service role only on transcription_consent_requests" ON call_intelligence.transcription_consent_requests FOR ALL TO authenticated,
anon USING (auth.role () = 'service_role')
WITH
  CHECK (auth.role () = 'service_role');

-- ==========================================
-- 3. Add Consent Status to Call Sessions
-- ==========================================
ALTER TABLE call_intelligence.telavox_call_sessions
ADD COLUMN IF NOT EXISTS consent_status text DEFAULT 'pending' CHECK (
  consent_status IN (
    'pending',
    'approved',
    'declined',
...
-- ==========================================
-- 4. Update Job Queue for New Job Types
-- ==========================================
...
-- ==========================================
-- 5. Add Settings for Consent Configuration
-- ==========================================
-- Ensure settings table exists (created in later migration, but needed here)
...
-- ALTER TABLE call_intelligence.telavox_call_sessions DROP COLUMN IF EXISTS consent_status;
-- ALTER TABLE call_intelligence.telavox_job_queue DROP CONSTRAINT IF EXISTS telavox_job_queue_job_type_check;
-- ALTER TABLE call_intelligence.telavox_job_queue ADD CONSTRAINT telavox_job_queue_job_type_check CHECK (job_type IN ('recording.lookup', 'stt.request', 'hubspot.sync'));
-- DELETE FROM call_intelligence.settings WHERE key IN ('consent_enabled', 'consent_timeout_hours', 'consent_reminder_hours', 'consent_auto_approve_known_contacts', 'slack_bot_token', 'slack_signing_secret');
