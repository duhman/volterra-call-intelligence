/**
 * Setup Telavox Org Config Script
 * Run this to configure your Telavox org in the database
 * 
 * Usage:
...
async function setupTelavoxConfig() {
  ...
  if (!webhookSecret) {
    console.error("Error: TELAVOX_WEBHOOK_SECRET environment variable is required");
    process.exit(1);
  }

  if (!accessToken) {
    console.error("Error: TELAVOX_ACCESS_TOKEN environment variable is required");
    console.error("This is different from TELAVOX_WEBHOOK_SECRET");
    console.error("Get your API access token from Telavox API settings");
    process.exit(1);
  }
  ...
  if (error) {
    console.error("Failed to setup Telavox config:", error);
    process.exit(1);
  }

  console.log("✅ Telavox org config updated successfully!");
  console.log(`   Org ID: ${data.telavox_org_id}`);
  console.log(`   HubSpot Portal ID: ${data.hubspot_portal_id}`);
  console.log(`   Webhook Secret: ${data.webhook_secret ? "✅ Set" : "❌ Missing"}`);
  console.log(`   Access Token: ${data.access_token ? "✅ Set" : "❌ Missing"}`);
}
