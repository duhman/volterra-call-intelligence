...
export async function syncTelavoxCallToHubSpot(
  ...
  const competitorMentions: string[] =
    (insights.competitorMentions as string[]) || [];
  ...
  const createEngagement = async () => {
      ...
      try {
        ...
        const callObject = await hs.crm.objects.calls.basicApi.create({
          properties: {
            ...
            hs_call_title: `AI Transcribed Call — ${new Date(occurredAt).toLocaleDateString()}`,
            hs_call_body: body,
            hs_call_duration: durationSeconds
              ? String(durationSeconds * 1000)
              : "", // milliseconds (empty string if not available)
    ...
    const note = await hs.crm.objects.basicApi.create("notes", {
      properties: {
        hs_note_body: body,
        hs_timestamp: occurredAt,
        hs_note_title: `AI Transcribed Call — ${occurredAt}`,
      },
    });
